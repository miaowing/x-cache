...
var assert = require('assert'),
    LruXCache = require('../dist/xcache.common').LruXCache;
...
describe('LRU Cache', function () {
    ...
    describe('#get(key, returnEntry)', function () {
        it('respond with matching records', function (done) {
            ...
            assert.equal(toString(cache), 'jerry:22 elwin:33 leon:44 tom:11 ');
            done();
        });
    });
    ...
    describe('#put(key, value)', function () {
        it('respond with matching records', function (done) {
            ...
            cache.put('test', 66);
            assert.equal(cache.getSize(), 4);
            assert.equal(toString(cache), 'test:66 sam:55 jerry:22 elwin:33 ');
            done();
        });
    });
    ...
    describe('#remove(key)', function () {
        it('respond with matching records', function (done) {
            ...
            assert.equal(toString(cache), 'test:66 jerry:22 elwin:33 ');
            done();
        });
    });
